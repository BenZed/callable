    ...
    it('can be bound', () => {
        class Shout extends Callable<() => string> {
            constructor(protected readonly _words: string) {
                super(function (this: any) {
                    const ctx = (this[AbstractCallable.context] ?? this) as {
                        _words: string
                    }
                    return `${ctx?._words}!`
                })
...
it('retrieve signature', () => {
    ...
    class Voider extends Callable<() => void> {
        constructor() {
            super(toVoid)
        }
    }
    ...
    expect(voider[AbstractCallable.signature]).toEqual(toVoid)
})
...
it('retrieve context', () => {
    class Voider extends Callable<() => void> {
        constructor() {
            super(function (this: any) {
                return this[AbstractCallable.context]
            })
        }
    }
